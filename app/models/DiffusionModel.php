...
class DiffusionModel
{
    ...
    function __construct()
    {
        ...
        $this->getDiffusions = $this->bdd->prepare("SELECT d.id as diffusion_id, d.*, f.id as film_id, f.* FROM `diffusion` d JOIN `film` f ON f.id = d.film_id WHERE date_diffusion >= CURDATE() ORDER BY date_diffusion ASC LIMIT :limit");
        $this->getNextDiffusions = $this->bdd->prepare("SELECT d.id as diffusion_id, d.*, f.id as film_id, f.* FROM `diffusion` d JOIN `film` f ON f.id = d.film_id WHERE date_diffusion >= CURDATE() ORDER BY date_diffusion ASC LIMIT 3");
        $this->getDiffusionsByFilmId = $this->bdd->prepare("SELECT d.id as diffusion_id, d.*, f.id as film_id, f.* FROM `diffusion` d JOIN `film` f ON f.id = d.film_id WHERE f.id = :id AND date_diffusion >= CURDATE() ORDER BY date_diffusion ASC");
